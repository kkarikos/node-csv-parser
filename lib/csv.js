...
module.exports = function(){
    ...
    /**
     * If the csv separator is automatic, try initializing it. 
     */
    function init(chars) {
        var quoteOpen = false;
        for(var i = 0; i < chars.length; i++) {
            ...
            switch(c) {
                ...
                default:
                    if(!quoteOpen) {
                        if(csv.readOptions.possibleDelimiters.indexOf(c) > -1) {
                            csv.readOptions.delimiter = c;
                            return;
                        }
                    }
            }
        ...
        throw { reason: 'No delimiter found from document.' };
    }
    ...
    function parse(chars){
        chars = '' + chars;
        if(csv.readOptions.delimiter == 'auto') {
            init(chars);
        }
